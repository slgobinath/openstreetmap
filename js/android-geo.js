function GeoMap(divId, latitude, longitude, zoom) {
    ...
    this.showTimeline = function(data) {
        // Move the map to keep the current location at the center
        this.center(data[0].geometry.coordinates[0][1], data[0].geometry.coordinates[0][0]);

        // Get start/end times
        var startTime = new Date(data[0].properties.time[0]);
        var endTime = new Date(data[0].properties.time[data[0].properties.time.length - 1]);
